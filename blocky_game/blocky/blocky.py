...
def _block_to_squares(board: Block) -> List[Tuple[Tuple[int, int, int],
    ...
    """
    # Base case, check if <board> has no children
    if not board.children:
        ...
        for child in board.children:
            board_lst.extend(_block_to_squares(child))
        return board_lst
...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the given <event> for the current player's turn.
        """
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Update the visual representation after a valid move is performed
    ...
    def render(self, renderer: Renderer) -> None:
        """Process the updated version of the board through <renderer>, and
        display it to Blocky's user interface.
        """
...
class AnimateMoveState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Ignore the parameter <event> and return None.
        """
        return  # Ignore the event

    def update(self) -> GameState:
        """Return the last GameState if the animation of processing a move has
        completed running. Otherwise, continue displaying the animaton.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        """Process and draw the animation, via <renderer>, of a player's move,
        and outline around the block that the move is being processed on.
        And update the status message of the player.
        """
...
class GameOverState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Ignore the parameter <event>, and do not process the event.
        """
        # Simply ignore the event
        return

    def update(self) -> GameState:
        """Return the GameState after the last turn has been executed.
        """
        # Nothing to change
        return self

    def render(self, renderer: Renderer) -> None:
        """Process and display the user interface of the board in Blocky after
        the game has finished using <renderer>.
        """
        x = 10
        y = 10
