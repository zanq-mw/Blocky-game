...
from player import _get_block, RandomPlayer, SmartPlayer, HumanPlayer
...
class TestPlayer:
    ...
    def test_get_block_bottom_left(self, board_16x16) -> None:
        """Test that the correct block is retrieved from the reference board
        when requesting the bottom-left corner of the board.
        """
        bottom_left = (0, board_16x16.size - 1)
        assert _get_block(board_16x16, bottom_left, 0) == board_16x16
        assert _get_block(board_16x16, bottom_left, 1) == board_16x16.children[2]
        assert _get_block(board_16x16, bottom_left, 2) == \
            board_16x16.children[2] # Works when children are defined
